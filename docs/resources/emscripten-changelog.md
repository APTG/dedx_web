...
Note that there is _no_ ABI compatibility guarantee between versions - the ABI
...
- In `-sMODULARIZE` mode the factory function will now always return a promise,
  ...
  consistency and simplicity we now _always_ return a promise here. (#24727)
...
- The `-sMAIN_MODULE=1` mode no longer exports all the main module symbols on
  ...
  that `-sMAIN_MODULE=1` includes _all_ native symbols in your program as well
...
- In `-sMODULARIZE` mode, the argument passed into the module constructor is
  ...
  on the object passed _into_ the constructor will now abort. (#21775)
...
- Fix generating of symbol files with `--emit-symbol-map` for JS targets.
  When `-s WASM=2` is used. Two symbols are generated: - `[name].js.symbols` - storing Wasm mapping - `[name].wasm.js.symbols` - storing JS mapping
...
- Added support to use a custom set of substitution characters . # and ? to
  ease passing arrays of C symbols on the command line to ASYNCIFY\_\* settings.
  (#13477)
...
- Breaking changes to calling Wasm function pointers from JavaScript:
  ...
  available in -s MINIMAL_RUNTIME=1 builds. 2. old syntax for calling a Wasm function pointer from a JS library file used
  to be
...
- Add `--extern-pre-js` and `--extern-post-js` emcc flags. Files provided there
  are prepended/appended to the final JavaScript output, _after_ all other
...
- New EM*IMPORT macro to mark C/C++ symbols as imported from outside the module
  (i.e. imported from JS). Currently we still default to assuming that \_all*
...
- Add LLD*REPORT_UNDEFINED option that should allow for more detailed
  ...
  are reported as undefined at link time, as are `\_\_invoke*\*` functions.
...
- Redefine errno values to be consistent with wasi. This will let us avoid
  ...
  unless you use errno defines (like EAGAIN) _and_ keep around binaries
...
- Options passed on the `Module` object during startup, like `Module.arguments`,
  ...
  modify `Module.arguments` and other things _after_ startup (which is now
...
- The transition also moves all builds and downloads away from the old
  ...
  to _mozilla-games_ URLs will not work (these were never documented, but could
...
- Fix `Module.locateFile` to resolve relative paths to _.wasm, _.mem and other
...
- Fixed a compiler error "unresolved symbol:
  \_\_cxa_decrement_exception_refcount" (#2715)
...
- Implemented new HTML5 API for direct WebGL context creation, emscripten*webgl*\*().
...
- Added new linker flag -s DEMANGLE_SUPPORT to choose whether to compile the
  application with libcxxabi-provided demangling support \_\_\_cxa_demangle().
...
- Added support for \_\_cxa_bad_typeid (#2547)
...
- Renamed the EM*ASM*() macro to the form EM_ASM_ARGS().
...
- Added new macro EM*ASM*, which allows sending values to JS without returning anything.
...
- _Numerous_ optimizations in both compilation and runtime stages.
...
- Fix issues with strnlen, memmove, LDBL\_ constants, va_args, float.h, and others.
