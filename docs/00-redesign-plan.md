...
| 6.2  | Live calculation (debounced reactive updates)                        | ✅     | [`calculator.md`](04-feature-specs/calculator.md)                                 | Stage 5.2 — `debounce` wired in `calculator.svelte.ts`                                                                                                                                                                                      |
...
| 6.5  | Shareable URLs — Calculator + Plot + Advanced mode round-trip        | ✅     | [`shareable-urls.md`](04-feature-specs/shareable-urls.md)                         | [#399](https://github.com/APTG/dedx_web/pull/399) (calc), [#394](https://github.com/APTG/dedx_web/pull/394) (plot), [#423](https://github.com/APTG/dedx_web/pull/423) (adv mode)                                                            |
...
| 6.7  | Build info badge (footer)                                            | ✅     | [`build-info.md`](04-feature-specs/build-info.md) Final v1                        | Commit hash + ISO date + branch in footer; `deploy.cjs` → `deploy.json` at build time. [log](ai-logs/2026-05-06-stage6-7-build-info.md)                                                                                                     |
...
| 6.11 | Export: advanced mode additions                                      | ✅     | [`export.md`](04-feature-specs/export.md) Final v6 §§1.1, 4.1, 6.1               | CSV configuration modal (Calculator + Plot, advanced only); PNG export (Plot, advanced only); Calculator advanced PDF metadata block. Dep: 6.8 ✅.                                                                                          |
...
> deferred to Stage 7. Spike 4 (`VERDICT.md`) must complete first — see Stage 2.5 above.

### Stage 7: E2E Tests, Polish & External Data ⏳

- **Who:** AI implements per sub-stage spec (one per session).
- **Input:** `docs/07-testing-strategy.md`, `docs/09-non-functional-requirements.md`, individual spec files.
- **Status:** Not started.

#### Sub-stages

| #   | Feature                                             | Status | Spec / Doc                                                                                          | Notes                                                                                                                                                                                     |
| --- | --------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7.1 | Playwright E2E suite with real WASM + CI step       | ⏳     | [`07-testing-strategy.md`](07-testing-strategy.md), [`08-deployment.md`](08-deployment.md)          | Full E2E suite against actual `libdedx.wasm`; add `pnpm exec playwright install --with-deps` + `pnpm exec playwright test` to `ci.yml`; WASM artifact download before `pnpm build` in CI |
| 7.2 | JSROOT plot styling polish                          | ⏳     | [`plot.md`](04-feature-specs/plot.md)                                                               | Physics-grade defaults: log–log axes, axis labels with units, 14 pt fonts, legend top-right, line width 2, color palette. Must specify exact values — not "make it pretty"               |
| 7.3 | Mobile responsive polish                            | ⏳     | [`09-non-functional-requirements.md`](09-non-functional-requirements.md)                            | Calculator + Plot + multi-program table on small screens; WCAG 2.1 AA touch targets; tested with Playwright device emulation                                                              |
| 7.4 | WASM error handling                                 | ⏳     | [`calculator.md`](04-feature-specs/calculator.md), [`06-wasm-api-contract.md`](06-wasm-api-contract.md) | Typed `LibdedxError` propagated to UI; user-visible error states for load failure, calculation failure, and invalid input; no silent swallowing                                           |
| 7.5 | External data (Zarr v3 `.webdedx` user-hosted data) | ⏳     | [`external-data.md`](04-feature-specs/external-data.md), [`shareable-urls-formal.md`](04-feature-specs/shareable-urls-formal.md) | **Gate:** Spike 4 `VERDICT.md` must pass first (see Stage 2.5). `extdata=` URL param, `ext-ref` entity grammar, user-hosted Zarr v3 data files                                           |
