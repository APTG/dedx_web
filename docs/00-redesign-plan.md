...
- **Status:** In progress. Stage 7.1 is implemented in [PR #459](https://github.com/APTG/dedx_web/pull/459); Stage 7.2 is complete; Stages 7.3–7.5 are not started.
...
| 7.2 | JSROOT plot styling polish                          | ✅     | [`plot.md`](04-feature-specs/plot.md)                                                               | Most styling (log-log defaults, axis labels with units, line width 2, color palette, grid, disabled scroll-zoom) was implemented in earlier stages. This stage added explicit 14 pt font sizes (`fLabelSize = fTitleSize = 0.04` NDC). No JSROOT canvas legend — series list below canvas serves as legend per `plot.md`. [log](progress/stage-7.2.md) |
