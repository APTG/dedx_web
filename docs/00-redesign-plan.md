...
| TBD                                               | Entity-selector chrome + state rework | 🚧     | Removed `recipe-bar.svelte` and the `picker-recipe-*` testids; added `activeTarget`, `expanded`, `across`, `multiSelected` to `EntitySelectionState` with `setActiveTarget` / `setExpanded` / `setAcross` / `toggleMulti` setters; introduced `advanced-toolbar.svelte` (Compare-across dropdown — Programs enabled, Materials/Particles disabled; Load-external + Explore-compat disabled; `↺` reset); introduced `multi-list.svelte` (SELECTED + AVAILABLE rows with `DEFAULT` badge — drag/keyboard reorder deferred); Program tab renders `<MultiList>` when `across === "program"` in Advanced; Material tab gains a coral `+ New custom material` pill in Advanced; tabs render a coral active-target underline and a red-dashed/`!`-badge empty-tab styling (`picker-tab-{id}-empty` testid). Persistent picker-level search row, load-external modal, external-sources move, multi-list reorder, and Materials/Particles compare-across wiring are tracked as separate follow-up issues. |
