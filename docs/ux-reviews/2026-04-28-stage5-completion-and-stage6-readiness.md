...
| D9  | When the entity selection is partial (e.g. only Particle picked), the preview series is cleared but the canvas just shows "No data series" with no hint about _what's missing_. Re-using the calculator's incomplete-selection helper text would help. | nice-to-have | Stage 6 polish  |
...
- However, **the sidebar's** internal `md:grid-cols-[1fr_2fr]`
  ...
  _landscape_. At 768 px the two panels each get ~365 px which is fine,
...
- E2E coverage of the plot page is currently 2 smoke tests + 9 pre-
  ...
  clean HEAD"). Stage 6's data-series-comparison work is _theoretically_
