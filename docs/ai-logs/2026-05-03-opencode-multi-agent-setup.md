...
4. **`svelte.json` PLGrid override**: Without this, `svelte-file-editor`
   silently tries the Anthropic API on every `.svelte` edit and fails. The
   `svelte.json` file (committed at repo root, read automatically by the
   `@sveltejs/opencode` plugin) points all subagent calls to
...
- **Stage**: tooling
- **Files changed**:
  - `svelte.json` (new) — PLGrid model override for `svelte-file-editor`
