...
| File | Date | Topic |
|------|------|-------|
| [2026-04-13-custom-compounds.md](2026-04-13-custom-compounds.md) | 13 Apr 2026 | custom-compounds.md Draft v1 + revision: compound library (localStorage, StoredCompound), modal editor (Formula + Weight-fraction modes, n_i = w_i / M_i conversion, live sum), program compatibility filter (Bragg additivity, Z-based element check, greyed-out with tooltip), URL params renamed ccomp_* → mat_*, inverse lookups resolved via C wrappers (getInverseStpCustomCompound / getInverseCsdaCustomCompound / getBraggPeakStpCustomCompound in dedx_extra.{h,c}), getPlotDataCustomCompound (JS-side), PDF elemental composition table (Symbol/Z/Atom count/Weight fraction/M_i), shareable-urls-formal.md updated to v6 (mat_* ABNF + conformance vectors 18–21), 06-wasm-api-contract.md updated with 4 new service methods |
