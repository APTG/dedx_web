...
> surfaced as a small "Calculated with **PSTAR** (auto-selected)" annotation
> near the results on both the Calculator and Plot pages, rather than as a
> control the user must operate. Keyboard tab-cycling, next-empty-tab advance,
> and the mobile search sheet all scope to the two visible tabs in Basic mode.
> Advanced mode keeps the full three-tab picker unchanged. Switching
> Advanced → Basic while the Program tab is active retargets to a visible tab.
...
> behind the scenes and shown as a "Calculated with … (auto-selected)"
> annotation near the results. Everything below describes the Advanced-mode tab.
...
- [x] Basic mode hides the Program tab entirely; the program is auto-selected
      and shown as a "Calculated with … (auto-selected)" annotation near the
      results on the Calculator and Plot pages (#816).
