...
| [advanced-options.md](advanced-options.md) | Draft v3 | Advanced Options accordion (density override, I-value override, aggregate state Gas/Condensed toggle, interpolation axis scale + method, MSTAR mode): density override for all material types (gas/solid/liquid), per-type ⓘ tooltip, density surfaced in header, interpolation split into Axis scale (Log-log/Lin-lin) + Method (Linear/Spline), URL params agg_state/interp_scale/interp_method/mstar_mode/density/ival |
