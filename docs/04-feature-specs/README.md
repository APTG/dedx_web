...
| [advanced-options.md](advanced-options.md) | Draft v2 | Advanced Options accordion (density override, I-value override, aggregate state Gas/Condensed toggle, interpolation mode, MSTAR mode): visibility gating, gas-density UX (sci notation, auto-format placeholder, ⓘ tooltip), density surfaced in header, unit-coupling rules, per-series freeze on Plot, URL params agg_state/interp/mstar_mode/density/ival |
