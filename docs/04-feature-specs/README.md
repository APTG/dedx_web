...
| [calculator.md](calculator.md) | Final v7 | Landing page: unified input/result table, debounced live calculation, per-row unit detection, compact entity selection, responsive layout, URL state, CSV + PDF export; aligned to canonical unit conversion contract |
...
| [plot.md](plot.md) | Final v4 | Plot page: multi-series JSROOT chart with canonical per-series density conversion, explicit calculator-vs-plot default split, normalized URL unit tokens, PNG/SVG/PDF/CSV export, external-data CSV rules (Case B + comment rows) |
...
| [export.md](export.md) | Final v2 | Authoritative export spec across all three pages: CSV (Calculator 5-col, Calculator advanced wide-table, Plot wide-format with external-data Case B + `#` comment rows), image export (PNG/SVG dropdown), PDF export (jsPDF, mode-sensitive: basic = date+URL; advanced = full metadata block with build info/particle Z+A/density/interpolation/browser+OS); filename conventions; accessibility contract |
