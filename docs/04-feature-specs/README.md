...
| [inverse-lookups.md](inverse-lookups.md) | Draft v1 | Advanced-mode inverse lookups on Calculator page: Inverse STP tab (both branches shown simultaneously — E low / E high — no-solution `—`; input mirrors forward output unit) and Inverse CSDA tab (inline length-suffix detection nm/µm/mm/cm/m; monotonic, unique inverse); energy output auto-scaled eV/keV/MeV/GeV; shared entity selection and Advanced Options; URL params imode/ivalues/iunit at canonical step 8 |
