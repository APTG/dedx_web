"""Write synthetic dataset to Zarr v3 — per-ion shard variant.

shards=(1, 379, 165) → one shard file per ion (287 files per quantity).
chunks=(1, 379, 165) → inner chunk = entire shard, no sub-index needed.
Output: data/srim_synthetic_per_ion.zarr/
"""
...
OUT_PATH = "data/srim_synthetic_per_ion.zarr"
PROGRAM = "srim-2013"
...
def main():
    ...
    file_count = sum(len(files) for _, _, files in os.walk(OUT_PATH))
    print(f"Done. {OUT_PATH} — {total/1e6:.1f} MB total, {file_count} files")
    # Sample first ion shard size
    shard0 = os.path.join(OUT_PATH, PROGRAM, "stp", "c", "0", "0", "0")
    if os.path.exists(shard0):
        print(f"  stp c/0/0/0 (H-1): {os.path.getsize(shard0)/1024:.1f} KB")
    shard_e = os.path.join(OUT_PATH, PROGRAM, "stp", "c", "286", "0", "0")
    if os.path.exists(shard_e):
        print(f"  stp c/286/0/0 (e-): {os.path.getsize(shard_e)/1024:.1f} KB")
