...
function trackOn(name: string) {
  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (h instanceof Headers) {
      ...
      range =
        (h as Record<string, string>)["Range"] ?? (h as Record<string, string>)["range"] ?? null;
    }
...
function buildHAR(): object {
  ...
  const entries = reqs.map((req) => {
    ...
    if (tao && p) {
      ...
      timings.ssl =
        p.secureConnectionStart > 0 ? Math.max(0, p.connectEnd - p.secureConnectionStart) : 0;
      timings.send = 0;
...
$run.addEventListener("click", async () => {
  ...
  try {
    ...
    appendLog(`\n${"─".repeat(75)}`);
    appendLog(
      "✓ Done — results rendered below.  Use 'Export HAR JSON' button to save timing data.",
    );
    $exp.disabled = false;
  } catch (err) {
    appendLog(`\n✗ ERROR: ${String(err)}`);
