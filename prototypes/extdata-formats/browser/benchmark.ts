...
function sz(bytes: number): string {
  return bytes < 1e6 ? `${(bytes / 1024).toFixed(1)} KB` : `${(bytes / 1e6).toFixed(2)} MB`;
}
...
async function benchPerIon(): Promise<{ rows: Row[]; stpText: string }> {
  ...
  rows.push({
    label: "zarr-per-ion / zarr.json (root)",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "particles + materials + energy grid",
  });
  ...
  rows.push({
    label: "zarr-per-ion / stp/zarr.json",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "array metadata",
  });
  ...
  rows.push({
    label: "zarr-per-ion / H-1 shard → water STP",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "downloads c/0/0/0 (full ion shard)",
  });
  stpText += fmtStp(
    protonStp as { data: Float32Array; shape: number[] },
    "H-1 in Water (per-ion Zarr)",
  );
  ...
  rows.push({
    label: "zarr-per-ion / C-12 shard → PMMA STP",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "downloads c/9/0/0 (full ion shard)",
  });
  stpText += fmtStp(
    carbonStp as { data: Float32Array; shape: number[] },
    "C-12 in PMMA (per-ion Zarr)",
  );
...
async function benchSingle(): Promise<{ rows: Row[]; stpText: string }> {
  ...
  console.log(`  zarr.json (root): ${sz(bytes)} in ${ms(elapsed)}`);
  rows.push({
    label: "zarr-single / zarr.json (root)",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "particles + materials + energy grid",
  });
  ...
  console.log(`  stp/zarr.json: ${sz(bytes)} in ${ms(elapsed)}`);
  rows.push({
    label: "zarr-single / stp/zarr.json",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "array metadata",
  });
  ...
  console.log(`  H-1 → WATER stp [${protonStp.shape}]: ${sz(bytes)} in ${ms(elapsed)}`);
  rows.push({
    label: "zarr-single / H-1 inner chunk → water STP",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "shard-index Range + inner-chunk Range",
  });
  stpText += fmtStp(
    protonStp as { data: Float32Array; shape: number[] },
    "H-1 in Water (single-shard Zarr)",
  );
  ...
  console.log(`  C-12 → PMMA stp [${carbonStp.shape}]: ${sz(bytes)} in ${ms(elapsed)}`);
  rows.push({
    label: "zarr-single / C-12 inner chunk → PMMA STP",
    sizeKb: bytes / 1024,
    elapsedMs: elapsed,
    note: "shard-index cached + inner-chunk Range",
  });
  stpText += fmtStp(
    carbonStp as { data: Float32Array; shape: number[] },
    "C-12 in PMMA (single-shard Zarr)",
  );
...
function buildReport(
  ...
  function coldTotal(rows: Row[], labels: string[]): [number, number] {
    ...
    return [subset.reduce((a, r) => a + r.sizeKb, 0), subset.reduce((a, r) => a + r.elapsedMs, 0)];
  }
