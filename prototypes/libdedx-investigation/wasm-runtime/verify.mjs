...
check(
  "ESTAR (id=3) in program list",
  programIds.includes(3),
...
check(
  "DEFAULT (100) covers ≥ 270 materials",
  (matCounts[100] ?? 0) >= 270,
...
check("ESTAR (3) material count = 0 (unimplemented)", matCounts[3] === 0, `${matCounts[3]}`);
console.log();
...
for (const { id, fragment } of ionChecks) {
  ...
  check(
    `Ion ${id} name${fragment ? ` contains "${fragment}"` : " lookup (electron)"}`,
    pass,
    `got "${name}"`,
  );
}
...
const stp = m.ccall(
  "dedx_get_simple_stp_for_program",
  "number",
...
check(
  "PSTAR H₂O 100 MeV/nucl — within 5% of 7.3 MeV·cm²/g",
  stpOk,
...
for (const { id, name, expected_eV } of iValueMaterials) {
  ...
  const withinTolerance =
    !Number.isFinite(deltaPercent) || Math.abs(deltaPercent) <= I_VALUE_TOLERANCE_PERCENT;
  const pass = iErr === 0 && iVal > 0 && withinTolerance;
  console.log(
    `  ${name.padEnd(12)} (id=${String(id).padStart(3)}): ${iVal.toFixed(1)} eV  (expected ~${expected_eV} eV, Δ=${delta}%, err=${iErr})`,
  );
  check(
  ...
  runtimeStats.i_value_checks.push({
    material_id: id,
    name,
    value_eV: iVal,
    expected_eV,
    delta_percent: Number.isFinite(deltaPercent) ? parseFloat(delta) : null,
    error_code: iErr,
  });
}
console.log();
...
for (const { id, name, expected_g_cm3 } of densityMaterials) {
  ...
  const density = m.ccall(
    "dedx_internal_read_density",
    "number",
    ["number", "number"],
    [id, dErrPtr],
  );
  ...
  console.log(
    `  ${name.padEnd(12)} (id=${String(id).padStart(3)}): ${density.toExponential(4)} g/cm³  (expected ~${expected_g_cm3}, err=${dErr})`,
  );
  check(`Density ${name} returns valid value`, pass, `${density.toExponential(4)} g/cm³`);
  runtimeStats.density_checks.push({
    material_id: id,
    name,
    value_g_cm3: density,
    expected_g_cm3,
...
for (const { id, name, expected_elements } of compMaterials) {
  ...
  console.log(
    `  ${name} (id=${id}): ${compLen} elements, Z=[${zValues.join(",")}], Σfrac=${fracSum.toFixed(4)}, err=${compErr}`,
  );
  for (const c of composition) {
    console.log(`    Z=${c.Z}, mass_fraction=${c.mass_fraction.toFixed(6)}`);
  }
  check(
    `Composition ${name} has expected elements`,
    hasExpected && compErr === 0,
    `Z=[${zValues.join(",")}]`,
  );
  ...
  runtimeStats.composition_checks.push({
    ...
    element_count: compLen,
    fraction_sum: fracSum,
    error_code: compErr,
  });
}
console.log();
...
console.log(
  `  ESTAR status : NOT IMPLEMENTED in libdedx v1.4.0 (code returns DEDX_ERR_ESTAR_NOT_IMPL)`,
);
...
const serviceMethods = [
  {
    ...
    cFunctions: ["dedx_fill_program_list", "dedx_get_program_name", "dedx_get_program_version"],
  },
  {
    name: "getParticles",
    category: "Entity Lists",
  ...
  {
    name: "calculate",
    category: "Stopping Power",
    cFunctions: ["dedx_get_stp_table", "dedx_get_csda_range_table"],
  },
  {
    name: "calculateCustomCompound",
    category: "Stopping Power",
    cFunctions: [
  ...
  {
    name: "getNucleonNumber",
    category: "Material Properties",
    cFunctions: ["dedx_get_ion_nucleon_number"],
  },
  {
    name: "getAtomicMass",
    category: "Material Properties",
    cFunctions: ["dedx_get_ion_atom_mass"],
  },
...
for (const method of serviceMethods) {
  const cFunctionsAvailable = method.cFunctions.every(
    (fn) => tsContractStats.exported_functions[fn]?.exported === true,
  );
  console.log(
    `  ${method.name.padEnd(30)} (${method.category.padEnd(18)}) ${cFunctionsAvailable ? "✓" : "✗"} C backing: [${method.cFunctions.join(", ") || "JS-only"}]`,
  );
...
try {
  ...
  progListValues = Array.from(
    m.HEAP32.subarray(progListPtrTS >> 2, (progListPtrTS >> 2) + programListLength),
  );
} finally {
...
checkTS(
  "dedx_fill_program_list fills output array",
  progListOk,
  `${populatedPrograms.length} populated entries`,
);
...
const wsFreeOk =
  tsContractStats.exported_functions["dedx_free_config"]?.exported === true &&
...
console.log(
  `  Exported C functions    : ${Object.values(tsContractStats.exported_functions).filter((f) => f.exported).length}/${Object.keys(tsContractStats.exported_functions).length}`,
);
console.log(
  `  Runtime methods         : ${Object.values(tsContractStats.runtime_methods).filter((m) => m.available).length}/${Object.keys(tsContractStats.runtime_methods).length}`,
);
console.log(
  `  LibdedxService methods  : ${Object.values(tsContractStats.libdedx_service_coverage).filter((m) => m.c_functions_available || m.c_functions.length === 0).length}/${Object.keys(tsContractStats.libdedx_service_coverage).length}`,
);
